...
async function handleInitialSearch(event) {
  ...
  if (!timeline) {
    setStatus('Select when care is needed.', true);
    timelineInput.focus();
    return;
  }
  if (!contactEmail) {
...
form?.addEventListener('submit', handleInitialSearch);
...
(() => {
  try {
    ...
    if (/^\d{5}$/.test(zip)) zipInput.value = zip;
  } catch (_err) {
    // ignore
  }
})();
